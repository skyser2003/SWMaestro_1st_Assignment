...
>
  ...
  <xsl:template match="*">
    <xsl:message terminate="yes">
      Node not handled: <xsl:for-each select="ancestor-or-self::*">
  ...
  <xsl:template name="toPascalCase">
    ...
    <xsl:if test="$value != ''">
      ...
      <xsl:choose>
        <xsl:when test="$segment != ''">
          <xsl:value-of select="translate(substring($segment,1,1),$alpha,$ALPHA)"/>
          <xsl:value-of select="substring($segment,2)"/>
          <xsl:if test="$keepDelimiter">
            <xsl:value-of select="$delimiter"/>
          </xsl:if>
          <xsl:call-template name="toPascalCase">
